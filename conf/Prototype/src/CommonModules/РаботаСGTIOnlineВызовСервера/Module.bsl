...
// Функция - Получить структуру URI
//
// Параметры:
//  СтрокаURI	 - Строка - Адрес сервиса
// 
// Возвращаемое значение:
//  Структура - Схема в адресе, Имя сервера, Хост и Порт
//
...
// Функция - Получить wml значение запроса
//
// Параметры:
//  Режим1_uidWell	 - Строка - Содержит параметр для подставления в отбор запроса по коду скважины
// 
// Возвращаемое значение:
//  Строка - Текст запроса для получения модели 
//
Функция Получить_wml_Значение(Режим1_uidWell) Экспорт 
	
	ТекстЗапроса = "";
	Макет = ПолучитьОбщийМакет("РаботаСGTIOnline_ШаблоныЗапросов");
	
	НайденнаяОбласть = Макет.Области.Найти("Режим1_Getwellbore"); 
	Если НайденнаяОбласть = Неопределено Тогда 
		ЗаписьЖурналаРегистрации(НСтр("ru='Не найден макет с шаблонами запросов для работы с GTI-Online.'"), 
									УровеньЖурналаРегистрации.Ошибка,
									Метаданные.ОбщиеМакеты.РаботаСGTIOnline_ШаблоныЗапросов);	
	...
	Иначе
		ТекстЗапроса = НайденнаяОбласть.Текст;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "$uidWell$"    , Режим1_uidWell);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "$uidWellbore$", "");

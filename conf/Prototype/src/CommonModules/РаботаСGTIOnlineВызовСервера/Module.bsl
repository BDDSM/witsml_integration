		...
		Если ЭлементКоллекции.ТипУзла = УсловиеСравнения И ЭлементКоллекции.ИмяУзла <> "logCurveInfo" И ЭлементКоллекции.ИмяУзла <> "uom" Тогда 	
			...
			Если Не ПустаяСтрока(Результат.АльтернативнаяТаблицаДляВыводаНаФорму) Тогда 
				Если ЭлементКоллекции.ИмяУзла = "data" Тогда 
					...
					масИзСтроки = СтрРазделить(ЭлементКоллекции.ТекстовоеСодержимое, ",", Истина);
					
					СтруктураЗаписи = Новый Структура();
					СтруктураЗаписи.Вставить("index"	  , ЭлементКоллекции.Атрибуты[0].Значение);
					СтруктураЗаписи.Вставить("measurement", масИзСтроки[0]);
					СтруктураЗаписи.Вставить("value"	  , масИзСтроки[1]);
					МассивСодержание.Добавить(СтруктураЗаписи);
					//XXX Костыль, для обработки операций в журнале операции

...
Процедура wellboresПриАктивизацииСтроки(Элемент)
	...
	ПодключитьОбработчикОжидания("ОбработкаОжидания_ПриАктивизацииСтроки_wellbores", 0.5, Истина);	
...
Процедура ПолучитьШаблонЗапроса_ПоКоманде(Знач ИмяМетода, Знач Значение_WMLtypeIn)
	...
	СтруктураПолученияЗапроса = Новый Структура("Значение_WMLtypeIn, Парам1, Парам2", Значение_WMLtypeIn, Элементы.Декорация_wells.Заголовок, Элементы.Декорация_wellbores.Заголовок);
	
	СтруктураОтвета = РаботаСGTIOnlineВызовСервераПовтИсп.ОпределитьНастройкиЗапросаКМетоду_ПоШаблону(СтруктураПолученияЗапроса);		
...
Процедура ОбработкаОжидания_ПриАктивизацииСтроки_wells() Экспорт 
	...
	Если Элементы.wells.ТекущиеДанные <> Неопределено Тогда 
		Элементы.Декорация_wells.Заголовок = Элементы.wells.ТекущиеДанные.uid;
	КонецЕсли;
...
Процедура ОбработкаОжидания_ПриАктивизацииСтроки_wellbores() Экспорт
	...
	Если Элементы.wellbores.ТекущиеДанные <> Неопределено Тогда
		Элементы.Декорация_wellbores.Заголовок = Элементы.wellbores.ТекущиеДанные.uid;
	КонецЕсли;
